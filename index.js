...
app.put('/todone', async (req, res) => {
    ...
    await Todo.query(`UPDATE todofor SET status='done' WHERE (task=$1 AND id=$2 AND dateof=$3)`,[task.task, id, task.dateof]);
    // var result = await Todo.query(`SELECT * FROM users`);
    // res.redirect(`/${rows}`)
    // res.redirect(`/?q='rows'`)
    // res.send(task);
...
app.delete('/clean', async (req, res) => {
    ...
    await Todo.query(`DELETE FROM todofor WHERE (task=$1 AND id=$2 AND dateof=$3 and status='done')`,[task.task, id, task.dateof]);
    // var result = await Todo.query(`SELECT * FROM users`);
    // res.redirect(`/${rows}`)
    // res.redirect(`/?q='rows'`)
    // res.send(task);
...
app.get('/taketodos', async (req, res) => {
    ...
    const todo = (await Todo.query(`select task, dateof, status from todofor where id=$1 ORDER BY task ASC, dateof ASC`,[id]));
    // await Todo.query(`DELETE FROM todos`)
    res.json(todo.rows);
